"""
ViViT-B Full Fine-Tuning on SSv2
...
SAVE_DIR    = './checkpoints/vivit_fullfinetuned/'
RESULTS_DIR = './results/'
NUM_EPOCHS  = 35
BATCH_SIZE  = 16
LR          = 1e-4
...
def main():
    ...
    model.classifier = nn.Linear(in_features, 174)
    ...
    print(f'ViViT-B: {total:.1f}M total, {trainable:.1f}M trainable')
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch+1) % 5 == 0:
            ckpt_path = os.path.join(SAVE_DIR, f'vivit_fullfinetuned_ep{epoch+1}.pth')
    ...
    results = {
        'model': 'ViViT-B Full FT', 'dataset': 'SSv2',
    ...
    with open(os.path.join(RESULTS_DIR, 'vivit_fullfinetuned_results.json'), 'w') as f:
